...
public class Robot extends TimedRobot {
  ...
  public void teleopPeriodic() {
    if(operatorJoystick.getLeftX()==0){
      setArmMotor(-.05);
    }else{
      setArmMotor(operatorJoystick.getLeftX()*ARM_OUTPUT_POWER);
    }

    if (operatorJoystick.getCrossButtonPressed()) {
      // cube in
      intakePower = INTAKE_OUTPUT_POWER;
      intakeAmps = INTAKE_CURRENT_LIMIT_A;
      lastGamePiece = CUBE;
      System.out.println("cross");
    } else if(operatorJoystick.getCrossButtonReleased()){
      ...
      intakePower = -INTAKE_HOLD_POWER;
      intakeAmps = INTAKE_HOLD_CURRENT_LIMIT_A;
    } else if(operatorJoystick.getCircleButtonPressed()){
  ...
  //    }    
    ...
    if (driverJoystick.getRightTriggerAxis() != 0) {
        drive.setMaxOutput(1);
    } else {

        drive.setMaxOutput(0.5);
    }
     var xSpeed = m_speedLimiter.calculate(-driverJoystick.getLeftX());
    final var rot = m_rotLimiter.calculate(-driverJoystick.getRightY());
    if (rot != 0 && xSpeed == 0) {
        xSpeed = 0.01;
    }
